...
phone would simply never have offered the install. Files generated by
`scripts/make-icons.py` from `public/favicon.svg`, so the icon set cannot drift from
the mark in the tab.

### Installing it on the shop devices

Browser menu → Install. The installed app and the URL are the same program; the
service worker updates it by itself, so a device that was off when you deployed
picks up the change on the next visit with a connection.
